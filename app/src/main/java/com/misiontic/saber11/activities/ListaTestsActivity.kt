...
class ListaTestsActivity : AppCompatActivity(), TestAdapter.OnTestClickListener,
    ...
    private lateinit var preguntas: ArrayList<Pregunta>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        preguntas =
            intent.extras?.get("preguntas") as ArrayList<Pregunta> //Cambiar esto por que se cargue desde la nube por posibles agregaciones en tiempo real
    ...
    private fun newTest() {
        ...
        var preguntasTest = mutableListOf<Pregunta>()
        val range = 0 until preguntas.size
        for (i in 0..4) {
            val random = range.random()
            preguntasTest.add(preguntas[random])
        }
